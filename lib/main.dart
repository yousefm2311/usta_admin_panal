...
Future<String> _resolveInitialRoute(TokenStorage storage) async {
  ...
  // If we have a valid access token, keep using it
  if (_isTokenValid(access)) return '/dashboard';
  ...
  // No refresh token available: route to login if access token is missing or expired
  return '/login';
}
