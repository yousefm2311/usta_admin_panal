...
class _ArtisanDetailsViewState extends State<ArtisanDetailsView> {
  ...
  Widget _kycReviewCard(Map<String, dynamic> verification) {
    final isMobile = Responsive.isMobile(context);
    final status =
        (verification['verificationStatus'] ?? 'pending_documents').toString();
    final confidence = verification['confidence'];
    final attempts = verification['attempts'];
    final reason = (verification['rejectionReasonUserSafe'] ??
            verification['failureReason'] ??
            '')
        .toString();
    final category = (verification['rejectionCategory'] ?? '').toString();
    final reviewedAt = verification['reviewedAt'];
    final reviewedBy = verification['reviewedBy'];
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              _miniStat(
                'Status'.tr,
                _prettyKycStatus(status),
                icon: Icons.verified_user_outlined,
                color: _kycStatusColor(status),
              ),
              _miniStat(
                'Confidence',
                confidence == null ? '-' : _formatStatValue(confidence),
                icon: Icons.face_retouching_natural,
                color: Colors.teal,
              ),
              _miniStat(
                'Attempts',
                _formatStatValue(attempts),
          ...
          const SizedBox(height: AppSizes.md),
          _infoRow('Category', _kycCategoryLabel(category)),
          _infoRow('Reason', reason),
          _infoRow('Reviewed at', formatDateString(reviewedAt).toString()),
          _infoRow(
            'Reviewed by',
            reviewedBy is Map<String, dynamic>
          ...
          if (isMobile)
            Column(
              children: [
                _verificationImageTile(
                  title: 'ID Front',
                  imageUrl: controller.verificationImageUrl(artisanId, 'idFront'),
                ),
                const SizedBox(height: 12),
                _verificationImageTile(
                  title: 'ID Back',
                  imageUrl: controller.verificationImageUrl(artisanId, 'idBack'),
                ),
                const SizedBox(height: 12),
                _verificationImageTile(
                  title: 'Selfie',
                  imageUrl: controller.verificationImageUrl(artisanId, 'selfie'),
                ),
              ],
            )
          else
  ...
  Widget _kycStatusChip(String status) {
    final color = _kycStatusColor(status);
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: AppSizes.sm, vertical: 6),
      decoration: BoxDecoration(
        color: color.withOpacity(0.14),
  ...
  String _kycCategoryLabel(String category) {
  ...
  Future<void> _showRejectKycDialog() async {
    final safeReasonController = TextEditingController();
    final internalReasonController = TextEditingController();
